...
class ExportResourceView(views.APIView):
    ...
    def get(self, request, format=None):
        ...
        if (format == "json"):
            dataset_json = self.resource.export_as_generator_json(mandate_id=mandate_id, request_id=request_id)
            return StreamingHttpResponse(dataset_json, headers={
                'Content-Disposition': f'attachment; filename="{self.filename}.json"'
            }, content_type="application/json")
        elif (format == "csv"):
            dataset_csv = self.resource.export_as_generator_csv(mandate_id=mandate_id, request_id=request_id)
