...
class VoteGroupSerializer(CommonCachableSerializer):
    def calculate_cache_key(self, group):
        ...
        memberships_updated_at = list(
            vote.motion.session.organizations.first()
            .personmemberships_children.filter(role="voter")
            .active_at(vote.timestamp)
    ...
    def get_annotated_group_ballots(self, group):
        annotated_group_ballots = (
            self.get_group_ballots(group)
            .values("option")
            .annotate(option_count=Count("option"))
    ...
    def get_outliers(self, group):
        ...
        sum_of_all_ballots = sum(
            self.get_annotated_group_ballots(group).values_list(
                "option_count", flat=True
            )
        )
        ...
        filtered_group_ballots = self.get_group_ballots(group).exclude(
            option__in=["absent", max_option]
        )
        outliers = [ballot.personvoter for ballot in filtered_group_ballots]
    ...
    def get_max(self, group):
        ...
        if sum_of_all_ballots != 0:
            max_option_percentage = (
                self.get_annotated_group_ballots(group).first()["option_count"]
                * 100
                / sum(
                    self.get_annotated_group_ballots(group).values_list(
                        "option_count", flat=True
    ...
    def get_votes(self, group):
        group_votes_params = {
            ...
            for option_sum in self.get_annotated_group_ballots(group)
        }
        return {
            key: group_votes_params.get(key, 0)
...
class VoteSumsSerializer(CommonCachableSerializer):
    def calculate_cache_key(self, vote):
        ...
        # we have ballots and at least one of them has a personvoter
            ...
            latest_voter_timestamp = (
                vote.ballots.filter(personvoter__isnull=False)
                .order_by("-personvoter__updated_at")
                .values("personvoter__updated_at")
                .first()["personvoter__updated_at"]
            )

        # there are ballots so we need the latest ballot update
        latest_ballot_timestamp = (
            vote.ballots.all()
            .order_by("-updated_at")
            .values("updated_at")
            .first()["updated_at"]
        ...
        timestamp = max(
            [vote_timestamp, latest_voter_timestamp, latest_ballot_timestamp]
        )
...
class VoteGovernmentSideSerializer(CommonCachableSerializer):
    ...
    def get_group(self, vote):
        return {"name": self.context["gov_side"], "acronym": None, "slug": None}
...
class VoteSerializer(CommonSerializer):
    ...
    def get_government_sides(self, vote):
        ...
        coalition_organization_membership = (
            org.organizationmemberships_children.active_at(vote.timestamp)
        ...
        coaliton_serializer = VoteGovernmentSideSerializer(
            vote, context=coalition_context
        )
        ...
        opposition_serializer = VoteGovernmentSideSerializer(
            vote, context=opposition_context
        )
    ...
    def get_members(self, vote):
        ...
        cache_key = (
            f"SingleVoteMembers__{vote.id}__{timestamp.strftime('%Y-%m-%dT%H:%M:%S')}"
        )
    ...
    def get_documents(self, vote):
        serializer = LinkSerializer(
            vote.motion.links.all()
            .exclude(tags__name="vote-pdf")
            .exclude(note="analyse")
            .order_by("id"),
            many=True,
            context=self.context,
        )
...
class SessionVoteSerializer(CommonSerializer):
    ...
    def get_all_votes(self, obj):
        # obj is the vote
        serializer = VoteSumsSerializer(obj, context=self.context)
        return serializer.data
...
class BareVoteSerializer(SessionVoteSerializer):
    def get_session(self, obj):
        # obj is the vote
        serializer = SessionSerializer(obj.motion.session, context=self.context)
        return serializer.data
...
class SpeechVoteSerializer(CommonSerializer):
    ...
    def get_result(self, obj):
        # obj is the vote
        serializer = VoteStatsSerializer(obj, context=self.context)
        return serializer.data
